...
class Counter:
    ...
    def clear(s, vnums):  # reset all counters
        ...
        s.counters = {'note': dict(tups), 'nopr': dict(tups),
                      'nopt': dict(tups)}
...
class Music:
    ...
    def initVoices(s, newPart=0):
        ...
        for v in s.vnums:
            s.vtimes[
                v] = 0  # {voice: the end time of the last item in each voice}
    ...
    def insertElem(s, v,
                   elem):  # insert at the start of voice v in the current measure
        obj = Elem(elem)
    ...
    def appendObj(s, v, obj, dur):
        ...
        s.incTime(dur)
        if s.tijd > s.vtimes[v]: s.vtimes[
            v] = s.tijd  # don't update for inserted earlier items

    def appendElem(s, v, elem, tel=0):
        s.appendObj(v, Elem(elem), 0)
        if tel: s.cnt.inc('note',
                          v)  # count number of certain elements in each voice (in addition to notes)
    ...
    def getLastRec(s, voice):
        if s.gMaten: return s.gMaten[-1][voice][
            -1]  # the last record in the last measure
        return None  # no previous records in the first measure

    def getLastMelis(s, voice, num):  # get melisma of last measure
        if s.gLyrics:
            lyrdict = s.gLyrics[-1][
                voice]  # the previous lyrics dict in this voice
            if num in lyrdict: return lyrdict[num][
                1]  # lyrdict = num -> (lyric string, melisma)
        return 0  # no previous lyrics in voice or line number

    def addChord(s, note,
                 noot):  # careful: we assume that chord notes follow immediately
    ...
    def addBar(s, lbrk, m):  # linebreak, measure data
        if m.mdur and s.maxtime > m.mdur: info(
            'measure %d in part %d longer than metre' % (m.ixm + 1, m.ixp + 1))
        ...
        for v in s.vnums:
            ...
            s.appendElem(v,
                         ' %s' % m.rline)  # insert current barline record at time maxtime
            s.voices[v] = sortMeasure(s.voices[v],
                                      m)  # make all times consistent
            lyrs = s.lyrics[v]  # [{number: sylabe}, .. for all notes]
            lyrdict = {}  # {number: (abc_lyric_string, melis)} for this voice
            nums = [num for d in lyrs for num in
                    d.keys()]  # the lyrics numbers in this measure
            maxNums = max(
                nums + [0])  # the highest lyrics number in this measure
            for i in range(maxNums, 0, -1):
                xs = [syldict.get(i, '') for syldict in
                      lyrs]  # collect the syllabi with number i
                melis = s.getLastMelis(v, i)  # get melisma from last measure
                lyrdict[i] = abcLyr(xs, melis)
            s.lyrics[
                v] = lyrdict  # {number: (abc_lyric_string, melis)} for this measure
            mkBroken(s.voices[v])
    ...
    def outVoices(s, divs, ip, isSib):  # output all voices of part ip
        ...
        for iv in vnum_keys:
            if s.cnt.getv('note',
                          iv) == 0:  # no real notes counted in this voice
                continue  # skip empty voices
            if abcOut.denL:
                unitL = abcOut.denL  # take the unit length from the -d option
            else:
                unitL = compUnitLength(iv, s.gMaten,
                                       divs)  # compute the best unit length for this voice
            ...
            if s.repbra:
                if s.nvlt == 1 and s.vceCnt > 1: abcOut.add(
                    'I:repbra 0')  # only volta on first voice
                if s.nvlt == 2 and iv > lvc:     abcOut.add(
                    'I:repbra 0')  # only volta on first voice of each part
            if s.cpl > 0:
                s.bpl = 0  # option -n (max chars per line) overrules -b (max bars per line)
            elif s.bpl == 0:
            ...
            while vn:  # while still measures available
                ...
                chunk = vn[0]
                while ib < len(vn):
                    if s.cpl > 0 and len(chunk) + len(
                        vn[ib]) >= s.cpl: break  # line full (number of chars)
                ...
                lyrlines = sorted(
                    vl.items())  # order the numbered lyric lines for output
...
class ABCoutput:
    ...
    def __init__(s, fnmext, pad, X, options):
        ...
        s.volpan = int(
            options.m)  # 0 -> no %%MIDI, 1 -> only program, 2 -> all %%MIDI
        ...
        if len(options.p) == 7:
            for k, v in zip(s.pagekeys, options.p):
                try:
                    ...
                    info('illegal float %s for %s', (k, v)); continue

    def add(s, str):
        s.outlist.append(str + '\n')  # collect all ABC output

    def mkHeader(s, stfmap, partlist, midimap, vmpdct,
                 koppen):  # stfmap = [parts], part = [staves], stave = [voices]
        ...
        for i, k in enumerate(s.pagekeys):
            if s.jscript and k in ['pageheight', 'topmargin',
                                   'botmargin']: continue
            if s.pageFmt[k] != None: hd.append(
                '%%%%%s %.2f%s\n' % (k, s.pageFmt[k], i > 0 and 'cm' or ''))
        if staves and len(accStf) > 1: hd.append('%%score ' + staves + '\n')
        tempo = s.tempo and 'Q:%d/%d=%s\n' % (
        s.tempo_units[0], s.tempo_units[1],
        ...
        if s.jscript:
            defLs = sorted(d.items(), key=lambda x: (
            -x[1], x[0]))  # when tie (1) sort on key (0)
        else:
            defLs = sorted(d.items(), key=lambda x: -x[1])
        defL = s.denL and s.denL or defLs[0][
            0]  # override default unit length with -d option
        hd.append('L:1/%d\n%sM:%s\n' % (defL, tempo, s.mtr))
        ...
        for vnum, clef in s.clefs.items():
            ...
            dmap = midimap[vnum - 1][
                   4:]  # map of abc percussion notes to midi notes
            if dmap and 'perc' not in clef: clef = (
                        clef + ' map=perc').strip();
            ...
            for abcNote, step, midiNote, notehead in dmap:
                ...
                if abcMid(abcNote) != midiNote or abcNote != step:
                    if s.volpan > 0: hd.append(
                        '%%%%MIDI drummap %s %s\n' % (abcNote, midiNote))
                    hd.append('I:percmap %s %s %s %s\n' % (
                    abcNote, step, midiNote, notehead))
                    s.dojef = s.tstep
            if defL != s.cmpL[
                vnum - 1]:  # only if computed unit length different from header
                hd.append('L:1/%d\n' % s.cmpL[vnum - 1])
        s.outlist = hd + s.outlist
        if koppen:  # output SVG stuff needed for tablature
            k1 = kopSvg.replace('-2',
                                '-5') if s.shiftStem else kopSvg  # shift note heads 3 units left
    ...
    def writeall(
            s):  # determine the required encoding of the entire ABC output
        str = ''.join(s.outlist)
...
def abcdur(nx, divs,
           uL):  # convert an musicXML duration d to abc units with L:1/uL
...
def abcMid(note):  # abc note -> midi pitch
    ...
    acc, n, oct = r.groups()
    nUp = n.upper()
    p = 60 + [0, 2, 4, 5, 7, 9, 11]['CDEFGAB'.index(nUp)] + (
        12 if nUp != n else 0);
...
def staffStep(ptc, o, clef, tstep):
    ...
    if not tstep and clef.startswith(
        'bass'): ndif += 12  # transpose bass -> treble (C3 -> A4)
...
def setKey(fifths, mode):
    sharpness = ['Fb', 'Cb', 'Gb', 'Db', 'Ab', 'Eb', 'Bb', 'F', 'C', 'G', 'D',
                 'A', 'E', 'B', 'F#', 'C#', 'G#', 'D#', 'A#', 'E#', 'B#']
    offTab = {'maj': 8, 'ion': 8, 'm': 11, 'min': 11, 'aeo': 11, 'mix': 9,
              'dor': 10, 'phr': 12, 'lyd': 7, 'loc': 13, 'non': 8}
...
def outVoice(measure, divs, im, ip,
             unitL):  # note/elem objects of one measure in one voice
    ix = 0
    while ix < len(measure):  # set all (nested) tuplet annotations
        nx = measure[ix]
        if isinstance(nx, Note) and nx.fact and not nx.grace:
            ix, tupcnt = insTup(ix, measure, (
            1, 1))  # read one tuplet, insert annotation(s)
        ix += 1
    vs = []
    for nx in measure:
    ...
    while vs.find('!ped-up!!ped-up!') >= 0: vs = vs.replace('!ped-up!!ped-up!',
                                                            '!ped-up!')
    while vs.find('!8va(!!8va)!') >= 0: vs = vs.replace('!8va(!!8va)!',
                                                        '')  # remove empty ottava's
    return vs
...
def parseParts(xs, d, e):  # -> [elems on current level], rest of xs
    ...
    if x.tag == 'part-group':
        ...
        if type == 'start':  # go one level deeper
            s = [x.findtext(n, '') for n in
                 ['group-symbol', 'group-barline', 'group-name',
                  'group-abbreviation']]
            d[num] = s  # remember groupdata by group number
            e.append(num)  # make stack of open group numbers
            elemsnext, rest1 = parseParts(xs, d,
                                          e)  # parse one level deeper to next stop
            elems, rest2 = parseParts(rest1, d,
                                      e)  # parse the rest on this level
            ...
            if xs and xs[0].get('type') == 'stop':  # two consequetive stops
                if num != nums:  # in the wrong order (tempory solution)
                    d[nums], d[num] = d[num], d[
                        nums]  # exchange values    (only works for two stops!!!)
            sym = d[
                num]  # retrieve an return groupdata as last element of the group
            return [sym], xs
    else:
        elems, rest = parseParts(xs, d,
                                 e)  # parse remaining elements on current level
...
def prgroupelem(x, gnm, bar, pmap, accVce,
                accStf):  # collect partnames (accVce) and %%score map (accStf)
    if type(x) == tupletype:  # partname-tuple = (part-name, part-abbrev)
        y = pmap.pop(0)
        if gnm[0]: x = [n1 + ':' + n2 for n1, n2 in
                        zip(gnm, x)]  # put group-name before part-name
        accVce.append(x)
        accStf.extend(bracePart(y))
    elif len(x) == 2 and type(x[
                                  0]) == tupletype:  # misuse of group just to add extra name to stave
        y = pmap.pop(0)
        nms = [n1 + ':' + n2 for n1, n2 in zip(x[0], x[1][
                                                     2:])]  # x[0] = partname-tuple, x[1][2:] = groupname-tuple
        accVce.append(nms)
        accStf.extend(bracePart(y))
    else:
...
def prgrouplist(x, pbar, pmap, accVce,
                accStf):  # collect partnames, scoremap for a part-group
    sym, bar, gnm, gabbr = x[
        -1]  # bracket symbol, continue barline, group-name-tuple
    ...
    for uL in [4, 8, 16]:  # try 1/4, 1/8 and 1/16
        ...
        for im, m in enumerate(maten):  # all measures
            for e in m[iv]:  # all notes in voice iv
                if isinstance(e,
                              Elem) or e.dur == 0: continue  # no real durations
                vLen += len(
                    abcdur(e, divs[im], uL))  # add len of duration string
...
def doSyllable(syl):
    ...
    for e in syl:
        if e.tag == 'elision':
            ...
            txt += (e.text or '').replace('_', '\_').replace('-',
                                                             r'\-').replace(
                ' ', '~')
...
def checkMelismas(lyrics, maten, im, iv):
    ...
    for n, (lyrstr,
            melis) in prvlyr.items():  # all lyric numbers in the previous measure
        if n not in curlyr and melis:  # melisma required, but no lyrics present -> make one!
            ...
            if ms: curlyr[n] = (
            ms, 0)  # set melisma as the n-th lyrics of the current measure
...
def perc2map(abcIn):
    ...
    for x in ls:
        ...
        if 'V:' in x or 'K:' in x:
            ...
            if r: id = r.group(1)
            abc.append(x)
            if id in dmaps and len(dmaps[id]) > 0: abc.extend(dmaps[id]); del \
            dmaps[id]
            if 'perc' in x and 'map=' not in x: x += ' map=perc';
            if 'map=perc' in x and len(maps[id]) > 0: abc.append(
                '%%voicemap perc' + id);
...
class Parser:
    ...
    def __init__(s, options):
        ...
        s.midDflt = [-1, -1, -1,
                     -91]  # default midi settings for channel, program, volume, panning
    ...
    def matchSlur(s, type2, n, v2, note2, grace,
                  stopgrace):  # match slur number n in voice v2, add abc code to before/after
        if type2 not in ['start',
                         'stop']: return  # slur type continue has no abc equivalent
        if n == None: n = '1'
        if n in s.slurBuf:
            ...
            if type2 != type1:  # slur complete, now check the voice
                if v2 == v1:  # begins and ends in the same voice: keep it
                    if type1 == 'start' and (
                            not grace1 or not stopgrace):  # normal slur: start before stop and no grace slur
                        note1.before = [
    ...
    def doNotations(s, note, nttn, isTab):
        for key, val in s.ornaments:
            if nttn.find(key) != None: note.before += [
                val]  # just concat all ornaments
        ...
        if trem != None:
            ...
            if type == 'single':
                ...
                if s.tstep:  # abc2svg version
                    if type == 'stop': note.before.insert(0,
                                                          '!trem%s!' % trem.text);
                else:  # abc2xml version
                    if type == 'start': note.before.insert(0, '!%s-!' % (
                                int(trem.text) * '/'));
        fingering = nttn.findall('technical/fingering')
        for finger in fingering:  # handle multiple finger annotations
            if not isTab: note.before += [
                '!%s!' % finger.text]  # fingering goes before chord (addChord)
        ...
        if wvln != None:
            if wvln.get('type') == 'start':
                note.before = [
                                  '!trill(!'] + note.before  # keep left-right order!
        ...
        if glis != None:
            ...
            if glis.get('type') == 'start':
                note.before = [
                                  '!%s(!' % lt] + note.before  # keep left-right order!
    ...
    def tabnote(s, alt, ptc, oct, v, ntrec):
        ...
        for i in range(4):  # support same note on 4 strings
            na = s.note_alts[i % 3][
                p]  # get alternative representation of same note
            o = oct
        ...
        s.tabmap[
            v, nt] = ntrec.tab  # for tablature map (voice, note) -> (string, fret)
        return nt  # ABC code always in key C (with midi pitch alterations)

    def ntAbc(s, ptc, oct, note, v, ntrec,
              isTab):  # pitch, octave -> abc notation
        ...
        if ntrec.tab:
            return s.tabnote(alt, ptc, oct, v,
                             ntrec)  # implies s.tstep is true (options.t was given)
        elif isTab and s.tstep:
            nt = ['__', '_', '', '^', '^^'][int(alt or '0') + 2] + addoct(ptc,
                                                                          oct)
            info('no string notation found for note %s in voice %d' % (nt, v),
                 1)
        p = addoct(ptc, oct)
        if alt == None and s.msralts.get(ptc,
                                         0): alt = 0  # no alt but key implies alt -> natural!!
        if alt == None and (p,
                            v) in s.curalts: alt = 0  # no alt but previous note had one -> natural!!
        if acc == None and alt == None:
            ...
            alt = int(float(alt))
            if (p,
                v) in s.curalts:  # the note in this voice has been altered before
                if alt == s.curalts[
                    (p, v)]: return p  # alteration still the same
            elif alt == s.msralts.get(ptc, 0):
                return p  # alteration implied by the key
            tieElms = note.findall('tie') + note.findall(
                'notations/tied')  # in xml we have separate notated ties and playback ties
            if 'stop' in [e.get('type') for e in
                          tieElms]: return p  # don't alter tied notes
            info(
                'accidental %d added in part %d, measure %d, voice %d note %s' % (
                alt, s.msr.ixp + 1, s.msr.ixm + 1, v + 1, p))
        s.curalts[(p, v)] = alt
        p = ['__', '_', '=', '^', '^^'][
                alt + 2] + p  # and finally ... prepend the accidental
        return p

    def doNote(s, n):  # parse a musicXML note tag
        note = Note()
        v = int(n.findtext('voice', '1'))
        if s.isSib: v += 100 * int(
            n.findtext('staff', '1'))  # repair bug in Sibelius
        ...
        if e != None and e.text == 'none' and (
                not isTab or v in s.hasStems or s.tstep):
            note.before += ['s'];
            abcOut.stemless = 1;
        e = n.find('accidental')
        if e != None and e.get(
            'parentheses') == 'yes': note.ntdec += '!courtesy!'
        if r != None:
        ...
        if n.find('unpitched') != None:
            ...
            step = staffStep(p, int(o), clef,
                             s.tstep)  # (clef independent) step value of note on the staff
            ...
            if nh in ['circle-x', 'diamond',
                      'triangle']: noot = '_' + noot.replace('^', '').replace(
                '_', '')
            if nh and n.find('notehead').get('filled', '') == 'yes': nh += '+'
            if nh and n.find('notehead').get('filled', '') == 'no': nh += '-'
            s.drumNotes[(v, noot)] = (
            step, midi, nh)  # keep data for percussion map
        tieElms = n.findall('tie') + n.findall(
            'notations/tied')  # in xml we have separate notated ties and playback ties
        if 'start' in [e.get('type') for e in
                       tieElms]:  # n can have stop and start tie
            noot = noot + '-'
        note.beam = sum([1 for b in n.findall('beam') if
                         b.text in ['continue', 'end']]) + int(note.grace)
        lyrlast = 0;
        rsib = re.compile(r'^.*verse')
        for e in n.findall('lyric'):
            lyrnum = int(
                rsib.sub('', e.get('number', '1')))  # also do Sibelius numbers
            if lyrnum == 0:
        ...
        if chord:
            ...
            if s.curStf[v] != xmlstaff:  # the note should go to another staff
                ...
                s.msc.appendElem(v,
                                 '[I:staff %+d]' % dstaff)  # insert a move before the note
            s.msc.appendNote(v, note, noot)
        for slur in n.findall(
                'notations/slur'):  # s.msc.lastnote points to the last real note/chord inserted above
            s.matchSlur(slur.get('type'), slur.get('number'), v,
                        s.msc.lastnote, note.grace,
                        stopgrace)  # match slur definitions

    def doAttr(s, e):  # parse a musicXML attribute tag
        teken = {'C1': 'alto1', 'C2': 'alto2', 'C3': 'alto', 'C4': 'tenor',
                 'F4': 'bass', 'F3': 'bass3', 'G2': 'treble', 'TAB': 'tab',
                 'percussion': 'perc'}
        dvstxt = e.findtext('divisions')
        if dvstxt: s.msr.divs = int(dvstxt)
        steps = int(e.findtext('transpose/chromatic',
                               '0'))  # for transposing instrument
        fifths = e.findtext('key/fifths')
        first = s.msc.tijd == 0 and s.msr.ixm == 0  # first attributes in first measure
        if fifths:
            key, s.msralts = setKey(int(fifths),
                                    e.findtext('key/mode', 'major'))
        ...
        s.msr.mdur = (s.msr.divs * s.msr.mtr[0] * 4) // s.msr.mtr[
            1]  # duration of measure in xml-divisions
        ...
        for clef in e.findall('clef'):  # a part can have multiple staves
            n = int(
                clef.get('number', '1'))  # local staff number for this clef
            sgn = clef.findtext('sign')
            line = clef.findtext('line', '') if sgn not in ['percussion',
                                                            'TAB'] else ''
            cs = teken.get(sgn + line, '')
            oct = clef.findtext('clef-octave-change', '') or '0'
            if oct: cs += {-2: '-15', -1: '-8', 1: '+8', 2: '+15'}.get(
                int(oct), '')
            s.clefOct[n] = -int(
                oct);  # xml playback pitch -> abc notation pitch
            ...
            if stfdtl and int(stfdtl.get('number', '1')) == n:
                ...
                if strings:
                    tuning = [st.findtext('tuning-step') + st.findtext(
                        'tuning-octave') for st in strings]
            ...
            if first:
                s.clefMap[
                    n] = cs  # clef goes to header (where it is mapped to voices)
            else:
                ...
                for v in voices:
                    if n != s.curStf[v]:  # voice is not at its home staff n
                        dstaff = n - s.curStf[v]
                        s.curStf[
                            v] = n  # reset current staff at start of measure to home position
                        s.msc.appendElem(v, '[I:staff %+d]' % dstaff)
                    s.msc.appendElem(v, '[K:%s]' % cs)

    def findVoice(s, i, es):
        stfnum = int(
            es[i].findtext('staff', 1))  # directions belong to a staff
        ...
        for e in es[i + 1:]:  # or to the voice of the next note
            if e.tag == 'note':
                ...
                if s.isSib: v += 100 * int(
                    e.findtext('staff', '1'))  # repair bug in Sibelius
    ...
    def doDirection(s, e, i, es):  # parse a musicXML direction tag
        def addDirection(x, vs, tijd, stfnum):
            ...
            for v in vs:
                if tijd != None:  # insert at time of encounter
                    s.msc.appendElemT(v, x.replace('(', ')').replace('ped',
                                                                     'ped-up'),
                                      tijd)
                else:
        ...
        def startStop(dtype, vs, stfnum=1):
            ...
            type = t.get('type', '')
            k = dtype + t.get('number',
                              '1')  # key to match the closing direction
            if type in typmap:  # opening the direction
                ...
                if k in s.dirStk:  # closing direction already encountered
                    ...
                    if stype == 'stop':
                        ...
                        info(
                            ...
                            dtype, stype, s.msr.ixp + 1, s.msr.ixm + 1,
                            vs + 1))
                        s.dirStk[k] = (
                        (type, vs))  # remember voice and type for closing
                else:
                    s.dirStk[k] = (
                    (type, vs))  # remember voice and type for closing
            elif type == 'stop':
                if k in s.dirStk:  # matching open direction found
                    ...
                    if type == 'stop':
                        ...
                        x = typmap[type].replace('(', ')').replace('ped',
                                                                   'ped-up')
        ...
        jmps = [('dacapo', 'D.C.'), ('dalsegno', 'D.S.'), ('tocoda', 'dacoda'),
                ('fine', 'fine'), ('coda', 'O'), ('segno', 'S')]
        ...
        if t != None:
            ...
            if minst:
                ...
                vids = [v for v, id in s.vceInst.items() if
                        id == minst.get('id')]
                if vids: vs = vids[
                    0]  # direction for the indentified voice, not the staff
                parm, inst = ('program', str(int(prg) - 1)) if prg else (
                'channel', chn)
                if inst and abcOut.volpan > 0: s.msc.appendElem(vs,
                                                                '[I:MIDI= %s %s]' % (
                                                                parm, inst))
            tempo = t.get('tempo')  # look for tempo attribute
            if tempo:
                tempo = '%.0f' % float(
                    tempo)  # hope it is a number and insert in voice 1
        ...
        for dirtyp in dirtypes:
            units = {'whole': (1, 1), 'half': (1, 2), 'quarter': (1, 4),
                     'eighth': (1, 8)}
            ...
            if metr != None:
                ...
                if metr.find('beat-unit-dot') != None:
                    tempo_units = simplify(tempo_units[0] * 3,
                                           tempo_units[1] * 2)
                tmpro = re.search('[.\d]+', metr.findtext(
                    'per-minute'))  # look for a number
            ...
            if not allwrds: allwrds = dirtyp.findall(
                'rehearsal')  # treat rehearsal mark as text annotation
            for wrds in allwrds:
            ...
            if t != None: startStop('octave-shift', vs,
                                    stf)  # assume size == 8 for the time being
            t = dirtyp.find('pedal')
            if t != None and s.ped:
                if not s.pedVce: s.pedVce = vs
                startStop('pedal', s.pedVce)
            if dirtyp.findtext(
                'other-direction') == 'diatonic fretting': s.diafret = 1;
        if tempo:
            tempo = '%.0f' % float(
                tempo)  # hope it is a number and insert in voice 1
            if s.msc.tijd == 0 and s.msr.ixm == 0:  # first measure -> header
                ...
                tempo_units[0], tempo_units[1],
                tempo))  # otherwise -> 1st voice
        if wrdstxt: s.msc.appendElem(vs, '"%s%s"' % (plc, wrdstxt),
    ...
    def doHarmony(s, e, i, es):  # parse a musicXMl harmony tag
        ...
        short = {'major': '', 'minor': 'm', 'augmented': '+',
                 'diminished': 'dim', 'dominant': '7',
                 'half-diminished': 'm7b5'}
        accmap = {'major': 'maj', 'dominant': '', 'minor': 'm',
                  'diminished': 'dim', 'augmented': '+', 'suspended': 'sus'}
        modmap = {'second': '2', 'fourth': '4', 'seventh': '7', 'sixth': '6',
                  'ninth': '9', '11th': '11', '13th': '13'}
        altmap = {'1': '#', '0': '', '-1': 'b'}
        ...
        if kind in short:
            ...
            if kind.startswith(
                'sus'): kind, sus = '', kind  # sus-suffix goes to the end
        elif kind == 'none':
        ...
        for d in degrees:  # chord alterations
            kind += altmap.get(d.findtext('degree-alter'), '') + d.findtext(
                'degree-value', '')
        kind = kind.replace('79', '9').replace('713', '13').replace('maj6',
                                                                    '6')
    ...
    def doBarline(s, e):  # 0 = no repeat, 1 = begin repeat, 2 = end repeat
        ...
        if end != None:
            if end.get('type') == 'start':
                ...
                try:
                    list(
                        map(int, n.split(',')))  # should be a list of integers
                except:
    ...
    def doPartList(s,
                   e):  # translate the start/stop-event-based xml-partlist into proper tree
        for sp in e.findall('part-list/score-part'):
            ...
            for m in sp.findall('midi-instrument'):
                ...
                if pan >= -90 and pan <= 90:  # would be better to map behind-pannings
                    pan = (float(
                        x[3]) + 90) / 180 * 127  # xml between -90 and +90
                midi[m.get('id')] = [int(x[0]), int(x[1]), float(x[2]) * 1.27,
                                     pan]  # volume 100 -> midi 127
                up = m.findtext('midi-unpitched')
                if up: s.drumInst[m.get('id')] = int(
                    up) - 1  # store midi-pitch for channel 10 notes
    ...
    def mkTitle(s, e):
        def filterCredits(y):  # y == filter level, higher filters less
            ...
            for x in credits:  # skip redundant credit lines
                ...
                if y < 5 and (
                        x in composer or x in lyricist): continue  # almost sure skip
                if y < 4 and ((title and title in x) or (
                        mvttl and mvttl in x)): continue  # may skip too much
                if y < 3 and (
                        [1 for c in composer if c in x] or [1 for c in lyricist
                                                            if
                                                            c in x]): continue  # skips too much
                if y < 2 and re.match(r'^[\d\W]*$',
                                      x): continue  # line only contains numbers and punctuation
                cs.append(x)
            if y == 0 and (
                    title + mvttl): cs = ''  # default: only credit when no title set
            return cs
        ...
        for creator in e.findall('identification/creator'):
            if creator.text:
                if creator.get('type') == 'composer':
                    composer += [line.strip() for line in
                                 creator.text.split('\n')]
                elif creator.get('type') in ('lyricist', 'transcriber'):
                    lyricist += [line.strip() for line in
                                 creator.text.split('\n')]
        ...
        s.isSib = 'Sibelius' in (
                    e.findtext('identification/encoding/software') or '')
    ...
    def doDefaults(s, e):
        ...
        eks = [a + b for a, b in zip(eks,
                                     'page-height,page-width,left-margin,right-margin,top-margin,bottom-margin'.split(
                                         ','))]
        for i in range(6):
            v = d.findtext(eks[i])
            k = abcOut.pagekeys[
                i + 1]  # pagekeys [0] == scale already done, skip it
            if not abcOut.pageFmt[k] and v:
                try:
                    abcOut.pageFmt[k] = float(v) * xmlScale  # -> cm
                except:
                    info('illegal value %s for xml element %s',
                         (v, eks[i])); continue  # just skip illegal values
    ...
    def locStaffMap(s, part, maten):  # map voice to staff with majority voting
        ...
        for n in ns:  # count staff allocations for all notes
            ...
            if v not in vmap:
                ...
                d[sn] = d.get(sn,
                              0) + 1  # ++ number of allocations for staff sn
            x = n.find('instrument')
            if x != None: s.vceInst[v] = x.get('id')
            x, noRest = n.findtext('stem'), n.find('rest') == None
            if noRest and (not x or x != 'none'): s.hasStems[
                v] = 1  # XML voice v has at least one stem
        vks = list(vmap.keys())
    ...
    def addStaffMap(s,
                    vvmap):  # vvmap: xml voice number -> global abc voice number
        part = []  # default: brace on staffs of one part
        for stf, voices in sorted(
                s.stfMap.items()):  # s.stfMap has xml staff and voice numbers
            locmap = [vvmap[iv] for iv in voices if iv in vvmap]
            nostem = [(iv not in s.hasStems) for iv in voices if
                      iv in vvmap]  # same order as locmap
            if locmap:  # abc voice number of staff stf
                part.append(locmap)
                clef = s.clefMap.get(stf,
                                     'treble')  # {xml staff number -> clef}
                for i, iv in enumerate(locmap):
                    clef_attr = ''
                    if clef.startswith('tab'):
                        if nostem[
                            i] and 'nostems' not in clef: clef_attr = ' nostems'
                        if s.diafret and 'diafret' not in clef: clef_attr += ' diafret'  # for all voices in the part
                    abcOut.clefs[
                        iv] = clef + clef_attr  # add nostems when all notes of voice had no stem
    ...
    def addMidiMap(s, ip, vvmap):  # map abc voices to midi settings
        ...
        if instr.values():
            defInstr = list(instr.values())[
                0]  # default settings = first instrument
        else:
        ...
        for v, vabc in vvmap.items():  # xml voice num, abc voice num
            ...
            ds = [(nt, step, midi, head) for (vd, nt), (step, midi, head) in ks
                  if v == vd]  # map perc notes
            id = s.vceInst.get(v,
            ...
            if id in instr:  # id is defined as midi-instrument in part-list
                ...
                xs.append(
                    (vabc, defInstr + ds))  # only one instrument for this part
    ...
    def parse(s, fobj):
            ...
            while s.msr.ixm < len(maten):
                ...
                for i, e in enumerate(es):
                    if e.tag == 'note':
                        ...
                        s.doDirection(maat, i,
                                      es)  # sound element directly in measure!
                    elif e.tag == 'harmony':
                        s.doHarmony(e, i, es)
                    elif e.tag == 'barline':
        ...
        if vvmapAll:  # skip output if no part has any notes
            abcOut.mkHeader(s.gStfMap, partlist, s.midiMap, s.tabVceMap,
                            s.koppen)
            abcOut.writeall()
        else:
...
if __name__ == '__main__':
    ...
    parser.add_option("-c", action="store", type="int",
                      help="set credit text filter to C", default=0,
                      metavar='C')
    parser.add_option("-d", action="store", type="int", help="set L:1/D",
                      default=0, metavar='D')
    parser.add_option("-n", action="store", type="int",
                      help="CPL: max number of characters per line (default 100)",
                      default=0, metavar='CPL')
    parser.add_option("-b", action="store", type="int",
                      help="BPL: max number of bars per line", default=0,
                      metavar='BPL')
    parser.add_option("-o", action="store", help="store abc files in DIR",
                      default='', metavar='DIR')
    parser.add_option("-v", action="store", type="int",
                      help="set volta typesetting behaviour to V", default=0,
                      metavar='V')
    ...
    parser.add_option("--noped", action="store_false",
                      help="skip all pedal directions", dest='ped',
                      default=True)
    parser.add_option("--stems", action="store_true",
                      help="translate stem directions", dest='stm',
                      default=False)
    ...
    if options.d and options.d not in [2 ** n for n in range(10)]:
        parser.error('D should be on of %s' % ','.join(
            [str(2 ** n) for n in range(10)]))
    ...
    for X, fnmext in enumerate(fnmext_list):
        ...
        abcOut = ABCoutput(fnm + '.abc', pad, X,
                           options)  # create global ABC output object
